...
class ComputeManagerUnitTestCase(test.NoDBTestCase):
    ...
    def _do_test_set_admin_password_driver_error(self, exc,
        ...
        def do_test(driver_mock, elevated_mock):
            ...
            if (expected_exception == exception.SetAdminPasswdNotSupported or
                    expected_exception == exception.VirtTypeNotSupported or
                    expected_exception == exception.Invalid or
    ...
    def test_set_admin_password_driver_virt_type_not_supported(self):
        exc = exception.VirtTypeNotSupported()
        expected_exception = exception.VirtTypeNotSupported
        self._do_test_set_admin_password_driver_error(
            exc, vm_states.ACTIVE, None, expected_exception)

    def test_set_admin_password_guest_agent_no_enabled(self):
        exc = exception.QemuGuestAgentNotEnabled()
        expected_exception = exception.Invalid
        self._do_test_set_admin_password_driver_error(
            exc, vm_states.ACTIVE, None, expected_exception)
    ...
    def _do_test_set_keypair_driver_error(self, exc,
        ...
        def do_test(driver_mock, elevated_mock):
            ...
            if (expected_exception == exception.VirtTypeNotSupported or
                    expected_exception == exception.Invalid or
                    expected_exception == NotImplementedError):
                instance_save_mock.assert_called_once_with(
                    expected_task_state=task_states.UPDATING_KEYPAIR)
    ...
    def test_set_keypair_driver_virt_type_not_supported(self):
        exc = exception.VirtTypeNotSupported()
        expected_exception = exception.VirtTypeNotSupported
        self._do_test_set_keypair_driver_error(
            exc, vm_states.ACTIVE, None, expected_exception)

    def test_set_keypair_guest_agent_no_enabled(self):
        exc = exception.QemuGuestAgentNotEnabled()
        expected_exception = exception.Invalid
        self._do_test_set_keypair_driver_error(
            exc, vm_states.ACTIVE, None, expected_exception)
